...
class ConvFuserWithAttention(nn.Module):
    """???ConvFuser:??????? + ?????(CBAM??)"""
    ...
    def __init__(self,
                 ...
                 reduction: int = 16):
        super().__init__()
        self.in_channels = sum(in_channels) if isinstance(in_channels, list) else in_channels
        ...
        # ?????
        self.channel_attn = ChannelAttention(out_channels, reduction) if use_channel_attn else None
...
@MODELS.register_module()
class CrossModalAttentionFuser(nn.Module):
    """CrossFusion ??:LiDAR?Camera ????????"""
    ...
    def __init__(self,
        ...
        self.camera_proj = nn.Conv2d(camera_channels, out_channels, 1)
        self.query_conv = nn.Conv2d(out_channels, out_channels, 1)
        self.key_conv = nn.Conv2d(out_channels, out_channels, 1)
        self.value_conv = nn.Conv2d(out_channels, out_channels, 1)
        self.softmax = nn.Softmax(dim=-1)
        self.output_proj = nn.Conv2d(out_channels, out_channels, 1)

    def forward(self, bottom_up_feats: List[torch.Tensor]) -> torch.Tensor:
        assert len(bottom_up_feats) == 2
        lidar_feat, cam_feat = bottom_up_feats
        lidar = self.lidar_proj(lidar_feat)
        cam = self.camera_proj(cam_feat)
        B, C, H, W = cam.shape
        Q = self.query_conv(cam).view(B, C, -1).permute(0, 2, 1)
        K = self.key_conv(lidar).view(B, C, -1)
        V = self.value_conv(lidar).view(B, C, -1).permute(0, 2, 1)
        attn = self.softmax(torch.bmm(Q, K))
        fused = torch.bmm(attn, V).permute(0, 2, 1).view(B, C, H, W)
...
@MODELS.register_module()
class CrossModalAttentionFuserLite(nn.Module):
    """??? CrossModalAttentionFuser:????????"""
    ...
    def forward(self, bottom_up_feats: List[torch.Tensor]) -> torch.Tensor:
        ...
        B, C, H, W = cam.shape
        h_reduced, w_reduced = H // self.reduction_ratio, W // self.reduction_ratio

        # ??? Q/K/V ????,?????
        cam_small = F.adaptive_avg_pool2d(cam, output_size=(h_reduced, w_reduced))
        lidar_small = F.adaptive_avg_pool2d(lidar, output_size=(h_reduced, w_reduced))

        Q = self.query_conv(cam_small).view(B, C, -1).permute(0, 2, 1)  # [B, N, C]
        K = self.key_conv(lidar_small).view(B, C, -1)                   # [B, C, N]
        V = self.value_conv(lidar_small).view(B, C, -1).permute(0, 2, 1)  # [B, N, C]

        attn = self.softmax(torch.bmm(Q, K))                            # [B, N, N]
        fused = torch.bmm(attn, V).permute(0, 2, 1).view(B, C, h_reduced, w_reduced)
...
# @MODELS.register_module()
# class ConvFuserWithAttention1(nn.Module):  # ??????????????? 'type' ???
#     """???ConvFuser:??????? + ?????(CBAM??)+ ???????"""
#
#     def __init__(self,
#                  in_channels: Union[int, List[int]],
...
#
#         # ?????????????(?????)
#         self.in_channels_per_frame = sum(in_channels) if isinstance(in_channels, list) else in_channels
#         self.out_channels = out_channels
#         self.num_temporal_frames = num_temporal_frames
#
#         # ???????
#         # ??????????? (???????????? * ?????)
#         total_input_channels = self.in_channels_per_frame * self.num_temporal_frames
#
#         self.conv = nn.Sequential(
#             nn.Conv2d(total_input_channels, out_channels, 3, padding=1, bias=False),
#             nn.BatchNorm2d(out_channels),
#             nn.ReLU(inplace=True)
#         )
#
...
#         Args:
#             inputs (List[Tensor]): ??????????
#                                   ????????? (B, C, H, W) ?? num_temporal_frames=1?
#                                   ????????? (B, T, C, H, W) ?? num_temporal_frames > 1?
#
#                                   ??:
#                                   - ???? (num_temporal_frames=1): [RGB_tensor, LiDAR_tensor]
#                                     RGB_tensor: (B, C_rgb, H, W)
#                                     LiDAR_tensor: (B, C_lidar, H, W)
#                                   - ???? (num_temporal_frames > 1): [RGB_seq_tensor, LiDAR_seq_tensor]
#                                     RGB_seq_tensor: (B, T, C_rgb, H, W)
#                                     LiDAR_seq_tensor: (B, T, C_lidar, H, W)
#
#         Returns:
...
#             if inp.ndim == 4 and self.num_temporal_frames == 1:
#                 # ???4D?????????,????????? T=1
#                 processed_inputs.append(inp.unsqueeze(1))  # (B, C, H, W) -> (B, 1, C, H, W)
#             elif inp.ndim == 5 and inp.shape[1] == self.num_temporal_frames:
#                 # ???5D?????????,????
...
#                     raise ValueError(
#                         f"??????:? 'num_temporal_frames' ? 1 ?,???????4? (B, C, H, W)?"
#                         f"??? {i} ???????? {inp.ndim},??? {inp.shape}?"
#                     )
#                 else:
#                     raise ValueError(
#                         f"??????:? 'num_temporal_frames' > 1 ?,???????5? (B, T, C, H, W),"
#                         f"?????? T ({inp.shape[1]}) ??? 'num_temporal_frames' ({self.num_temporal_frames}) ???"
#                         f"??? {i} ???????? {inp.ndim},??? {inp.shape}?"
#                     )
#
#         # ??????? T ??(????)??(???????,???????)
#         first_t = processed_inputs[0].shape[1]
#         if not all(p_inp.shape[1] == first_t for p_inp in processed_inputs):
#             raise ValueError("??????:????????? (T) ?????")
#
#         # 2. ?????????
#         # ??,????????????????,?? (B, T, C_total, H, W)
#         # ??,?? inputs = [img_seq (B,T,3,H,W), lidar_seq (B,T,10,H,W)]
#         # ??????? (B, T, 13, H, W)
#
#         # ?????,? (B, T, C, H, W) ?? (B, H, W, T, C) ??????
#         inputs_permuted = [p_inp.permute(0, 3, 4, 1, 2) for p_inp in processed_inputs]
#
#         # ???????(C)??????,?? (B, H, W, T, C_total)
#         x_multimodal_concat = torch.cat(inputs_permuted, dim=-1)
#
#         # ?? (B, T, C_total, H, W)
#         x_multimodal_concat = x_multimodal_concat.permute(0, 3, 4, 1, 2)
#
#         # ????? T ????? C_total “??”???
#         # ?? x_multimodal_concat ???? (B, T, C_total, H, W)
#         B, T, C_total, H, W = x_multimodal_concat.size()
#         x = x_multimodal_concat.view(B, T * C_total, H, W)  # ???????????
#
#         # --- ??????? ---
#         # ?? x ???? (B, total_input_channels, H, W),total_input_channels = T * C_total
#         x = self.conv(x)
#
#         # --- ??????? ---
#         if self.channel_attn is not None:
...
@MODELS.register_module()
class ConvFuserWithAttention1(nn.Module):
    """????????????,????????"""
    ...
    def __init__(self,
                 ...
                 reduction: int = 16,
                 num_temporal_frames: int = 1,
                 auto_reshape: bool = True):  # ????reshape??
        super().__init__()

        # ????????
        if isinstance(in_channels, list):
            self.in_channels_per_frame = sum(in_channels)
            self.modal_channels = in_channels  # ????????
        else:
            self.in_channels_per_frame = in_channels
            self.modal_channels = [in_channels]
    ...
    def forward(self, inputs: List[Tensor]) -> Tensor:
        # ?????
        processed = []
        for i, (inp, modal_c) in enumerate(zip(inputs, self.modal_channels)):
            # ??????
            if self.auto_reshape:
                inp = self._auto_adjust_dims(inp, modal_c, i)
            else:
                self._validate_dims(inp, modal_c, i)

            processed.append(inp)

        # ?????
        x = torch.cat(processed, dim=2)  # (B,T,C_total,H,W)

        # ??????
        B, T, C, H, W = x.shape
        x = x.permute(0, 2, 1, 3, 4).reshape(B, -1, H, W)  # (B,C*T,H,W)
        x = self.conv(x)
    ...
    def _auto_adjust_dims(self, inp: Tensor, modal_c: int, idx: int) -> Tensor:
        """????????"""
        if inp.ndim == 4:
            if self.num_temporal_frames == 1:
                return inp.unsqueeze(1)
            else:
                # ????????
                if inp.shape[1] == modal_c * self.num_temporal_frames:
                    return inp.view(
                        inp.size(0),
                ...
                elif inp.shape[1] == modal_c:
                    return inp.unsqueeze(1).expand(
                        -1, self.num_temporal_frames, -1, -1, -1)

        elif inp.ndim == 5:
            if inp.shape[1] != self.num_temporal_frames:
                # ??????????
                if inp.shape[2] == self.num_temporal_frames:
                    return inp.permute(0, 2, 1, 3, 4)

        # ????????,??????
        self._validate_dims(inp, modal_c, idx)
        return inp

    def _validate_dims(self, inp: Tensor, modal_c: int, idx: int):
        """???????"""
        error_msg = (
            f"??{idx}?????????:\n"
            f"- 4D??: (B, {modal_c * self.num_temporal_frames}, H, W) ? "
            f"(B, {modal_c}, H, W)\n"
            f"- 5D??: (B, {self.num_temporal_frames}, {modal_c}, H, W)\n"
            f"????: {tuple(inp.shape)}"
        )

        if inp.ndim == 4:
            if not (inp.shape[1] in [modal_c, modal_c * self.num_temporal_frames]):
                raise ValueError(error_msg)
        elif inp.ndim == 5:
            if inp.shape[1] != self.num_temporal_frames or inp.shape[2] != modal_c:
                raise ValueError(error_msg)
        else:
            raise ValueError(error_msg)