...
class DynamicTokenSelector(nn.Module):
    ...
    def __init__(self, depth_channels, confidence_threshold=0.1, max_tokens_ratio=0.3):
        super().__init__()
        ...
        self.confidence_net = nn.Sequential(
            nn.Conv2d(depth_channels, 16, 1),
            nn.ReLU(),
            nn.Conv2d(16, 1, 1),
            nn.Sigmoid()
        )

    def forward(self, depth_dist):
        B, D, H, W = depth_dist.shape

        # 1. ?????
        confidence = self.confidence_net(depth_dist).squeeze(1)  # [B, H, W]

        # 2. ??????
        max_response = torch.max(depth_dist, dim=1)[0]
        peak_score = torch.sigmoid(max_response)

        # 3. ?????? (??40%???)
        sky_weight = torch.ones_like(confidence)
        sky_height = int(H * 0.4)
        sky_weight[:, :sky_height, :] = 0.2

        # 4. ????
        scores = 0.5 * confidence + 0.3 * peak_score + 0.2 * sky_weight

        # 5. Token??
        valid_mask = self._select_tokens(scores)

        return valid_mask, scores

    def _select_tokens(self, scores):
        B, H, W = scores.shape
        max_tokens = int(H * W * self.max_tokens_ratio)

        batch_masks = []
        for b in range(B):
            score_flat = scores[b].flatten()

            # ???? + Top-K??
            threshold_mask = score_flat > self.confidence_threshold
            if threshold_mask.sum() > max_tokens:
                _, topk_idx = torch.topk(score_flat, max_tokens)
            ...
            # ??????
            if mask.sum() < 50:
                _, top_idx = torch.topk(score_flat, 50)
                mask = torch.zeros_like(score_flat, dtype=torch.bool)
                mask[top_idx] = True

            batch_masks.append(mask.reshape(H, W))

        return torch.stack(batch_masks)


@MODELS.register_module()
class SparseDepthLSSTransform(BaseDepthTransform):
    """????????DepthLSS????"""
    ...
    def __init__(
        ...
        if downsample > 1:
            ...
            self.downsample = nn.Sequential(
                ...
                nn.BatchNorm2d(out_channels),
                nn.ReLU(True),
                nn.Conv2d(out_channels, out_channels, 3, stride=downsample,
    ...
    def get_cam_feats(self, x, d):
        ...
        depth = depth_logits.softmax(dim=1)  # [B*N, D, fH, fW]

        # ???????
        if self.enable_sparse:
            valid_mask, scores = self.token_selector(depth)  # [B*N, fH, fW]

            # ??????
            total_tokens = fH * fW
            selected_tokens = valid_mask.sum(dim=(1, 2)).float().mean()
            sparsity_ratio = 1.0 - (selected_tokens / total_tokens)
            ...
            if not self.training:
                # ?????????????
                valid_mask_expanded = valid_mask.unsqueeze(1).expand(-1, self.D, -1, -1)
                depth = depth * valid_mask_expanded.float()
    ...
    def get_sparse_loss(self):
        """?????????(??)"""
        if not self.enable_sparse or not hasattr(self, 'token_selector'):
            return torch.tensor(0.0, device=next(self.parameters()).device)

        # ???????????
        # ????????????
        sparsity = self.sparse_stats.get('sparsity_ratio', 0.0)
        target_sparsity = 0.7  # ??70%???

        sparse_loss = F.mse_loss(
            torch.tensor(sparsity),
            torch.tensor(target_sparsity)
        )

        return self.sparse_loss_weight * sparse_loss


# ????:??????
def get_sparse_view_transform_config():
    """???????view_transform??"""
    return dict(
        type='SparseDepthLSSTransform',  # ?? 'DepthLSSTransform'
        in_channels=128,
        out_channels=80,
        image_size=[256, 704],
        feature_size=[32, 88],
        ...
        max_tokens_ratio=0.25,  # ??25%?tokens
        sparse_loss_weight=0.01,  # ??????
    )


# ????????????
class SparseBEVFusionLoss(nn.Module):
    """????????BEVFusion????"""
    ...
    def forward(self, pred, target, model):
        # ????
        original_loss = self.original_loss_fn(pred, target)

        # ?????(??????)
        sparse_loss = 0
        if hasattr(model, 'view_transform') and hasattr(model.view_transform, 'get_sparse_loss'):
            sparse_loss = model.view_transform.get_sparse_loss()

        total_loss = original_loss + sparse_loss
